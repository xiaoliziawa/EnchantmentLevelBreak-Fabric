...
public class CEnchantCommand {
    ...
    private static int enchantItem(CommandContext<ServerCommandSource> context, int level) throws CommandSyntaxException {
        ...
        if (itemStack.isEmpty()) {
            context.getSource().sendError(new TranslatableText(TRANSLATION_PREFIX + "no_item"));
            return 0;
        }
        ...
        if (enchantment == null) {
            context.getSource().sendError(new TranslatableText(TRANSLATION_PREFIX + "invalid_enchantment", enchantmentName));
            return 0;
        }
        ...
        context.getSource().sendFeedback(
            new TranslatableText(TRANSLATION_PREFIX + "success", enchantment.getName(level)),
            true
        );
